...
Flat pnpm workspace. The Mermaid Theme Builder is the root application package, and all application source lives at the workspace root. `artifacts/mermaid-theme-builder/` is a thin Replit platform package containing registration metadata, a package shell, and a preview asset. No backend, no database, no API.
...
│   └── pages/                     ← Tab pages (Apply, Compose, Examples, Extract, Reference)
├── .agents/                       ← Committed agent memory and skill packages
├── .canvas/                       ← Replit Canvas asset metadata
├── public/                        ← Static assets (favicon, manifest, Open Graph image, etc.)
├── examples/                      ← 68 .mmd authoring reference files (dev only, not built)
...
└── artifacts/
    └── mermaid-theme-builder/     ← Replit platform package, not app source
...
- **Port**: 18624
- **Tech**: React 19.2.8, Vite 8.1.4, Tailwind CSS 4.3.2, TypeScript 7.0.2, Mermaid.js 11.16.0
- **Version maintenance**: See `docs/technology-inventory.md` for the verified host inventory, complete package report, and daily audit plus Dependabot update process.
- **Git synchronization**: Follow `docs/github-sync.md` to publish reviewed changes and verify Replit, GitHub, and Windows parity. Use `corepack pnpm` for the pinned package manager.
...
- `src/pages/tabs/*.tsx` — Apply, Compose, Examples, Extract, and Reference tabs

**Authoring references (`examples/`):**
- 68 `.mmd` source files - authoring references for diagram content inlined into source data modules
...
`scripts/post-merge.sh` runs `corepack pnpm install --frozen-lockfile` after every task agent merge.
