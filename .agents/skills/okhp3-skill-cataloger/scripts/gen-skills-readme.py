...
def format_display_datetime(value: datetime) -> str:
    return value.strftime("%B %d, %Y at %H:%M UTC").replace(" 0", " ", 1)
...
def _record(skill_dir: Path, skill_md: Path, fm: dict, category: str, base: Path) -> dict:
    ...
    return {
        ...
        "path":          str(skill_md.relative_to(base)).replace("\\", "/"),
...
def write_family_md(
    ...
    now_disp = format_display_datetime(now)
    n        = len(skills)
...
def main() -> int:
    ...
    if args.full:
        ...
        now_disp = format_display_datetime(datetime.now(timezone.utc))
