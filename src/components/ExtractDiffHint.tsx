...
export function ExtractDiffHint() {
  ...
  return (
    <div
      ...
      role="note"
      aria-label="How to use the Extract Diff panel"
    >
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <p className="forge-eyebrow text-sky-700 dark:text-sky-300/80 mb-1.5">Reading the diff</p>
          <ol className="flex flex-col gap-0.5 sm:flex-row sm:gap-4">
            {(
              [
                ["1", "Paste a themed diagram above and click Extract theme"],
                ["2", "Pick a palette — the theme variables update the re-themed output"],
                ["3", "Lines in red were removed; lines in green are the new theme additions"],
              ] as [string, string][]
            ).map(([num, label]) => (
        ...
        <button
          ...
          aria-label="Dismiss Extract Diff panel hint"
