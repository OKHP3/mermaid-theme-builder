...
Replit platform registration file. The surrounding artifact directory also
contains a package shell and preview asset, but no application source code.
Do not delete the artifact registration or its supporting package files.
...
|   |   |-- technology-version-audit.yml scheduled dependency and runtime audit
|   |-- dependabot.yml
|   |-- FUNDING.yml
...
|       +-- package.json              Replit workspace package shell
|       +-- public/opengraph.jpg      Replit artifact preview asset
|-- .canvas/                  Replit Canvas asset metadata
...
|-- .agents/
|   +-- skills/
|       +-- okhp3-mermaid-theme-builder/  headless skill package for this app
...
GitHub Pages deployment is handled by `.github/workflows/deploy-pages.yml`. Base path
...
Current state as surveyed 2026-07-13.

| Directory | Current state | Notes |
| ----------------------------- | --------------------------------------------- | --------------------------------------------------- |
| `src/` | Full application source | All app code; see structure above |
| `src/__tests__/` | 75 unit and component test files | Hand-authored; none are generated |
| `src/__tests__/__snapshots__/` | 5 snapshot files | Generated; safe to delete and regenerate |
| `src/components/` | 16 React components | All PascalCase .tsx |
...
| `src/lib/` | 23 pure logic modules | All kebab-case .ts |
...
| `docs/` | 31 top-level files plus `roadmap/` | Human-authored product, design, process, and research documentation |
| `e2e/` | 23 Playwright spec files | All hand-authored |
| `examples/` | 68 `.mmd` files plus `emulated/` | Authoring references; content is inlined in source data modules |
| `public/` | Static web assets, including 3 generated PNGs | PNGs are generated by scripts/generate-icons.mjs |
| `scripts/` | 13 maintenance scripts | Includes the technology-version audit and E2E wrapper |
| `.github/workflows/` | 8 workflows | CI, deployment, E2E, links, skill tests, token sync, and version audit |
| `skills/okhp3-mermaid-theme-builder/` | SKILL.md, tests, assets, references, scripts | Full headless skill package; tested via skill-tests.yml |
| `standards/` | 2 Markdown docs | mermaid-theme-builder-standard.md, render-safety-checklist.md |
| `artifacts/mermaid-theme-builder/` | artifact registration, package shell, and preview asset | Replit platform metadata; not application source |
| `.canvas/` | 1 tracked asset | Replit Canvas metadata; not application source |
...
- **Identity and purpose, confirmed:** Mermaid Theme Builder is a personal
  OverKill Hill P3 browser tool for applying renderer-aware visual themes to
  Mermaid diagrams and exporting usable Mermaid, Markdown, and prompt-scaffold
  artifacts.
- **Primary users and value, confirmed/inferred:** People authoring Mermaid by
  hand or with AI use the tool to improve color governance, preview rendering,
  and reduce repeated style-correction prompts. This user-value statement is
  inferred from the README, product brief, and implemented workflow.
- **Mission, inferred:** Make Mermaid diagram styling more deliberate and
  portable across renderers without requiring server-side processing.
...
- **Architecture, confirmed:** The root package is the application. The
  ...
  processing is browser-only, with local browser persistence and no backend,
  database, login, AI API, or analytics transport for pasted diagram content.
- **Core workflow, confirmed:** paste Mermaid -> detect family -> select or edit
  a palette and renderer profile -> preview original and themed output -> copy
  or export.
- **Known guidance gap:** The project-local `skills/` tree is the canonical
  distribution source for the headless skill package. The `.agents/skills/`
  tree contains agent support packages and a generated catalog. Do not assume
  unrelated skill packages are synchronized. The link-check script scans the
  project-local distribution tree.
...
- Mermaid is used as an npm dependency (currently `11.17.2` -- pinned exactly).
