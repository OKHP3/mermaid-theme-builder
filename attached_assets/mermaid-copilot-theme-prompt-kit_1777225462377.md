# Mermaid Theme Prompt Kit for M365 Copilot

## Purpose

Reusable prompt components to produce visually consistent Mermaid diagrams from Microsoft 365 Copilot. Functions like a PowerPoint template — same styling, different content.

---

## Part 1: Theme Definition Blocks

Two formats are provided. **YAML frontmatter** is the current Mermaid standard (v10.5+). **Init directive** is deprecated but still functional and may render more reliably in Microsoft Loop, which sometimes lags behind the latest Mermaid version.
...
config:
  ...
  themeVariables:
    ...
    secondaryColor: "#89C2D9"
    secondaryTextColor: "#1B3A4B"
    secondaryBorderColor: "#5FA8D3"
    tertiaryColor: "#A9D6E5"
    tertiaryTextColor: "#1B3A4B"
    tertiaryBorderColor: "#89C2D9"
    lineColor: "#5FA8D3"
    textColor: "#333333"
    mainBkg: "#1B3A4B"
    nodeBorder: "#065A82"
    clusterBkg: "#E8F4F8"
    clusterBorder: "#89C2D9"
    ...
    signalColor: "#333333"
    labelBoxBkgColor: "#89C2D9"
    labelBoxBorderColor: "#5FA8D3"
...
```
%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#1B3A4B', 'primaryTextColor': '#FFFFFF', 'primaryBorderColor': '#065A82', 'secondaryColor': '#89C2D9', 'secondaryTextColor': '#1B3A4B', 'secondaryBorderColor': '#5FA8D3', 'tertiaryColor': '#A9D6E5', 'tertiaryTextColor': '#1B3A4B', 'tertiaryBorderColor': '#89C2D9', 'lineColor': '#5FA8D3', 'textColor': '#333333', 'mainBkg': '#1B3A4B', 'nodeBorder': '#065A82', 'clusterBkg': '#E8F4F8', 'clusterBorder': '#89C2D9', 'edgeLabelBackground': '#FFFFFF', 'noteBkgColor': '#FFF8E1', 'noteTextColor': '#333333', 'noteBorderColor': '#FFD54F', 'actorBkg': '#1B3A4B', 'actorBorder': '#065A82', 'actorTextColor': '#FFFFFF', 'signalColor': '#333333', 'labelBoxBkgColor': '#89C2D9', 'labelBoxBorderColor': '#5FA8D3', 'fontFamily': 'Segoe UI, Calibri, Arial, sans-serif', 'fontSize': '14px'}}}%%
```
...
| Role | Hex | Usage |
|------|-----|-------|
| Primary | `#1B3A4B` | Main node backgrounds (dark navy) |
| Primary Border | `#065A82` | Main node borders |
| Primary Text | `#FFFFFF` | Text on primary backgrounds |
| Secondary | `#89C2D9` | Supporting nodes, labels |
| Secondary Border | `#5FA8D3` | Supporting node borders, lines |
| Tertiary | `#A9D6E5` | Tertiary nodes, alternating fills |
| Cluster BG | `#E8F4F8` | Subgraph backgrounds |
| Note BG | `#FFF8E1` | Note/callout backgrounds (warm) |
| Note Border | `#FFD54F` | Note borders (amber accent) |
| Text | `#333333` | General text, edge labels |
| Highlight | `#E76F51` | Reserved for classDef emphasis |
...
1. Begin every diagram with this YAML frontmatter — do NOT modify it:
...
1. Start with this exact YAML frontmatter — include it verbatim, do not modify:
...
2. Do NOT add colors, styles, or gradients beyond what the frontmatter defines.
...
6. Verify before output: every subgraph has an end keyword, no orphan nodes, no duplicate IDs.
```

### Template C — Init Directive Variant (Loop Fallback)

If YAML frontmatter fails to render in Loop, use this alternate version in either Template A or B. Replace the frontmatter instruction with:

```
1. Start the diagram with this exact init directive on the very first line — include it verbatim:

%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#1B3A4B', 'primaryTextColor': '#FFFFFF', 'primaryBorderColor': '#065A82', 'secondaryColor': '#89C2D9', 'secondaryTextColor': '#1B3A4B', 'secondaryBorderColor': '#5FA8D3', 'tertiaryColor': '#A9D6E5', 'tertiaryTextColor': '#1B3A4B', 'tertiaryBorderColor': '#89C2D9', 'lineColor': '#5FA8D3', 'textColor': '#333333', 'mainBkg': '#1B3A4B', 'nodeBorder': '#065A82', 'clusterBkg': '#E8F4F8', 'clusterBorder': '#89C2D9', 'edgeLabelBackground': '#FFFFFF', 'fontFamily': 'Segoe UI, Calibri, Arial, sans-serif', 'fontSize': '14px'}}}%%
...
| `fontFamily` | trebuchet ms, verdana, arial | Font stack |
...
To swap in your own brand colors while keeping the template structure:

1. Choose your primary brand color (darkest, used for main nodes)
2. Choose 2-3 lighter tints of the same hue family for secondary/tertiary
3. Choose one contrasting warm accent for notes or highlights
4. Replace ONLY the hex values in the frontmatter block — keep all variable names unchanged
5. Use the Mermaid Live Editor (mermaid.live) to test before deploying to Copilot

The `base` theme will auto-calculate any variables you leave out, deriving them from `primaryColor`. So at minimum you only need to set `primaryColor`, `primaryTextColor`, and `lineColor` — but explicitly setting more gives you tighter control and prevents the engine from generating colors that clash with your palette.
